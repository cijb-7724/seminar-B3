...
public:
   ...
   void Qlaern(const vector<vector<double>> rw,const vector<vector<bool>> wall,const pair<int,int> goal,double alpha,double eps,int episodes);
};
...
void ActionValue::Qlaern(const vector<vector<double>> rw,const vector<vector<bool>> wall,const pair<int,int> goal,double alpha,double eps,int episodes){
   random_device rd;
   mt19937 mt(rd());
   uniform_int_distribution<int> dist03(0,3);
   uniform_real_distribution<double> dist01(0.0,1.0);
...
int main(){
   ...
   int episodes=100000;
   av.Qlaern(reward,wall,goal,ALPHA,EPS2,episodes);
   av.show();
