...
//p1-p2,p3-p4が交わるかどうか
bool is_cross(const Point p1,const Point p2,const Point p3,const Point p4){
   Point v1=p2-p1;
   Point v2=p4-p3;
   double x = ((p3-p1)*v1)/(v1*v1);
   double y = ((p1-p3)*v2)/(v2*v2);
   Point nearest_p1 = p1 + v1*x;
   Point nearest_p2 = p3 + v2*y;
   if (nearest_p1==nearest_p2 && 0+EPS<x && x<1-EPS && 0+EPS<y && y<1-EPS){
      return true;
   }
   else{
      return false;
